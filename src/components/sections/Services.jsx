...
const Services = () => {
  // State to track if all services should be shown
  const [showAll, setShowAll] = useState(false);

  // Determine which services to display based on the state
  ...
  return (
    <section className="relative py-6 lg:py-8 overflow-hidden bg-[#f8fbff]">
      <Container>
        {/* Top Header */}
        <div className="flex flex-row items-center justify-between gap-6 mb-6 lg:mb-10">
          <div>
            <h2 className="text-3xl md:text-5xl font-semibold text-[#0B2A4A] leading-tight">
              Core Services
            </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {visibleServices.map((item, index) => (
            <div
              key={item.id}
              className={`group relative h-full transition-all duration-500 hover:-translate-y-3 animate-fade-in-up`}
              // Adding a slight delay based on index for a staggered effect
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              {/* Glow Effect */}
              <div className="absolute inset-0 rounded-[30px] bg-gradient-to-br from-blue-500/10 to-indigo-500/10 blur-xl opacity-0 group-hover:opacity-100 transition-all duration-500" />
        ...
        {servicesData.length > 3 && (
          <div className="flex justify-center mt-10 lg:mt-14">
            <button
              ...
              className="group flex items-center gap-3 px-8 py-3 rounded-full bg-[#0B2A4A] text-white font-medium shadow-lg transition-all duration-300 hover:scale-105 hover:[#0B2A4A]"
            >
