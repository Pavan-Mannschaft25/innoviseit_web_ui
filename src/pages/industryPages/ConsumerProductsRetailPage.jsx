  ...
  <section className="lg:min-h-[90vh] flex flex-col lg:flex-row items-center justify-between px-6 lg:px-20 py-16 bg-primary-800 text-white relative overflow-hidden">
    ...
    >
      ...
      <div className="flex flex-wrap gap-3 mb-10">
        {[
          { icon: FaBoxes, label: "Supply Chain" },
          { icon: FaShoppingCart, label: "Omnichannel" },
          { icon: FaChartLine, label: "Analytics & AI" },
          { icon: FaUsers, label: "Customer Experience" },
        ].map((pill, i) => (
          <motion.div
            key={i}
          ...
          >
            <pill.icon className="text-[#FFD700] text-xs" />
            {pill.label}
          </motion.div>
        ))}
      </div>
        ...
        {steps.map((step, i) => (
          ...
          >
            <div className="relative inline-block mb-6">
              ...
              <div className="absolute -top-1 -right-1 w-8 h-8 bg-gradient-to-br from-[#FFD700] to-[#D4AF37] rounded-full flex items-center justify-center text-[#0a1628] text-xs font-bold shadow-lg border-2 border-white">
                {step.step}
              </div>
            </div>
